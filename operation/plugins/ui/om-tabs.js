...
(function(){
    ...
    $.omWidget('om.omTabs', {
        options : /** @lends omTabs#*/ {
			...
			onRefresh : function(tabId,event) {
            }
        },
	...
	//            omtabs.children(':first').css('width',options.width);
	    ...
	    /**
	     * 增加一个tab到页签布局中.最后一个参数isAjax指示了ds是否为一个URL  ,李 onRefresh 参数是新添加 为了，在关闭tab的时候可以刷新自定义的数据
	     */
    	_add : function(config/*title, content, url, closable , index, tabId, activateNew,onRefresh */) {
	        var _self = this,
	        	$self = this.element,
	            options = this.options,
	            items = this.items;
			...
			$.extend(cfg, config);
			if (config.onRefresh){
				options.onRefresh=config.onRefresh;  //此处为 添加的关闭事件 config 会多了一个参数 onRefresh 2013-08-03 李
			}
    	...
    	_close : function(index) {
			...
			options.onRefresh && this._trigger("onRefresh",null,index);
    	},
